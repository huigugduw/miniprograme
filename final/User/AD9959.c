 ...
 *
 * DDS_SCLK   -> AD9959_SCLK PA7
 * DDS_SDIO0  -> AD9959_SDIO0 PB2
 * DDS_CS     -> AD9959_CS PB3
 * DDS_UPDATE -> AD9959_UPDATE PA8
 * DDS_RST    -> AD9959_RST PA9
 * GND        -> GND
 *
 * AD9959模块其余数据引脚均接GND
*/

uint8_t FR1_DATA[3] = {0xD0,0x00,0x00}; // VCO增益控制[23]=1系统时钟高于255MHz; PLL[22:18]=10100,20倍频,20 * 25M=500MHz; Charge pump control = 75uA

uint8_t FR2_DATA[2] = {0x00,0x00};     // 双向扫描，即从起始值扫描到结束值后，又从结束值扫描到起始值
//uint8_t FR2_DATA[2] = {0x80,0x00};    // 单向扫描，即从起始值扫描到结束值后，又从起始值扫描到结束值，以此类推
...
** 函数功能：写LSRR线性扫描斜率寄存器
** 入口参数：    rsrr:    上升斜率,范围：0~255，系统时钟为500Mhz时一个控制字约为8ns
                            fsrr:    下降斜率,范围：0~255
** 出口参数：无
** 函数说明：无
**************************************************************/
...
void AD9959_writeProfileFreq(uint8_t profile, uint32_t data) {
    ...
    uint32_t Temp;
    Temp = (uint32_t)data * ACC_FRE_FACTOR;       //将输入频率因子分解为四个字节  4.294967296=(2^32)/500000000
...
**注意！！！：设置为4电平调制时，输出通道只能选择CH0-1
                            设置为8,16电平调制时，输出通道只能选择CH0
                            本函数未做任意通道兼容，具体方法请参AD9959芯片手册22-23页，操作FR1[14:12]为对应值。
**************************************************************/
